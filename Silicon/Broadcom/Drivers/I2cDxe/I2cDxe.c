 ...
 )
{
  ...
  //todo fetch base clock from rpi firmware
//  I2cClock = gPlatformGetClockPpi.PlatformGetClock (BCM_I2C_CLOCK, 0);
  I2cClock = 50000;
    ...
    )
{
  ...
  while ((MmioRead32 (I2cBase + BCM2835_I2C_S) & Mask) != Mask) {
    Retry++;
    if (Retry==10000) {
//    DEBUG ((DEBUG_ERROR, "PollMask=%X Controller status=%X control=%X\n", Mask, MmioRead32 (I2cBase + BCM2835_I2C_S), MmioRead32 (I2cBase + BCM2835_I2C_C)));
    }
    if (Retry==100000) {
//    DEBUG ((DEBUG_ERROR, "Timeout PollMask=%X Controller status=%X control=%X\n", Mask, MmioRead32 (I2cBase + BCM2835_I2C_S), MmioRead32 (I2cBase + BCM2835_I2C_C)));
      break;
    }
  }
}
...
//  DEBUG ((DEBUG_ERROR, "I2C operation to slave=%x read=%d bytes %d\n", SlaveAddress,Operation->Flags & I2C_FLAG_READ, Operation->LengthInBytes));
//  DEBUG ((DEBUG_ERROR, "0 Controller status=%X control=%X CDIV=%X\n",MmioRead32 (I2cBase + BCM2835_I2C_S), MmioRead32 (I2cBase + BCM2835_I2C_C),MmioRead32 (I2cBase + BCM2835_I2C_DIV)));
...
//  DEBUG ((DEBUG_ERROR, "I2C request to i2c device @ %X operations = %d\n", I2cBase, RequestPacket->OperationCount));
